...
void SortingAndFilteringScene::Filtering()
{
	...
	{
		std::sort(displayWeapons.begin(), displayWeapons.end(),
			[this](const Weapon* lhs, const Weapon* rhs)
			{
				switch (sortType)
				{
				case SORT_NAME:		return strcmp(lhs->name, rhs->name) < 0;
				case SORT_RARITY:	return lhs->rarity < rhs->rarity;
				case SORT_PRICE:	return lhs->price < rhs->price;
				case SORT_POWER:	return lhs->power < rhs->power;
				}
				return false;
			}
		);
