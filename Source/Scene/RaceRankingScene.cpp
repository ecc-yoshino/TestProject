...
void RaceRankingScene::Update(float elapsedTime)
{
	...
	for (Car& car : cars)
	{
		...
		// TODO①:チェックポイントを通過したか判定せよ
		{
			...
			DirectX::XMVECTOR NextCheckPointToOldCarVec = DirectX::XMVectorSubtract(OldCarPosition, NextCheckPointPosition);
			DirectX::XMVECTOR NextCheckPointToNowCarVec = DirectX::XMVectorSubtract(NowCarPosition, NextCheckPointPosition);
			DirectX::XMVECTOR NextCheckPointToOldCarDot = DirectX::XMVector3Dot(NextCheckPointDirection, NextCheckPointToOldCarVec);
			DirectX::XMVECTOR NextCheckPointToNowCarDot = DirectX::XMVector3Dot(NextCheckPointDirection, NextCheckPointToNowCarVec);
			float nextCheckPointToOldCarDot = DirectX::XMVectorGetX(NextCheckPointToOldCarDot);
			float nextCheckPointToNowCarDot = DirectX::XMVectorGetX(NextCheckPointToNowCarDot);
			if (nextCheckPointToOldCarDot * nextCheckPointToNowCarDot < 0.0f)
			{
				car.nowCheckPointIndex = (car.nowCheckPointIndex + 1) % checkPoiontCount;
				car.checkCount++;
			}
		}
		// TODO②:チェックポイント間の進行距離を算出せよ
		{
			...
			DirectX::XMVECTOR NowCheckPointPosition = DirectX::XMLoadFloat3(&nowCheckPoint.position);
			DirectX::XMVECTOR NowCheckPointDirection = DirectX::XMLoadFloat3(&nowCheckPoint.direction);
			DirectX::XMVECTOR NowCheckPointToNowCarVec = DirectX::XMVectorSubtract(NowCarPosition, NowCheckPointPosition);
			DirectX::XMVECTOR NowCheckPointToNowCarDot = DirectX::XMVector3Dot(NowCheckPointDirection, NowCheckPointToNowCarVec);
			car.progress = DirectX::XMVectorGetX(NowCheckPointToNowCarDot);
		}
	}
