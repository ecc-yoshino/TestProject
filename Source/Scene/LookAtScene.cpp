...
void LookAtScene::Update(float elapsedTime)
{
	...
	{
		// ローカル空間変換
		DirectX::XMVECTOR WorldTargetPosition = DirectX::XMLoadFloat3(&targetPosition);
		DirectX::XMMATRIX HeadWorldTransform = DirectX::XMLoadFloat4x4(&headNode.worldTransform);
		DirectX::XMMATRIX InverseHeadWorldTransform = DirectX::XMMatrixInverse(nullptr, HeadWorldTransform);
		DirectX::XMVECTOR LocalVec = DirectX::XMVector3Transform(WorldTargetPosition, InverseHeadWorldTransform);
		DirectX::XMVECTOR LocalDirection = DirectX::XMVector3Normalize(LocalVec);
		DirectX::XMVECTOR LocalForward = DirectX::XMLoadFloat3(&headLocalForward);
		...
		if (DirectX::XMVector3NotEqual(LocalAxis, DirectX::XMVectorZero()))
		{
			...
			if (angle > 0)
			{
				...
				DirectX::XMVECTOR LocalRotation = DirectX::XMLoadFloat4(&headNode.rotation);
				LocalRotation = DirectX::XMQuaternionMultiply(LocalRotationAxis, LocalRotation);
				DirectX::XMStoreFloat4(&headNode.rotation, LocalRotation);
